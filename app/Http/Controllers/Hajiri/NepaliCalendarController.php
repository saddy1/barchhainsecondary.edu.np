...
class NepaliCalendarController  extends Controller{
    public $bs = array(
    ...
    private $eng_date = array('year'=>'', 'month'=>'', 'date'=>'', 'day'=>'','nmonth'=>'','num_day'=>'');
    public $debug_info = "";

    public function __construct()
    {
        $this->loadDatabaseCalendarYears();
    }

    private function loadDatabaseCalendarYears(): void
    {
        try {
            if (! Schema::hasTable('nepali_calendar_years')) {
                return;
            }

            NepaliCalendarYear::query()
                ->orderBy('bs_year')
                ->get()
                ->each(function (NepaliCalendarYear $year) {
                    $months = $year->monthDays();

                    if (count($months) !== 12) {
                        return;
                    }

                    $this->bs[$year->bs_year - 2000] = array_merge([$year->bs_year], $months);
                });

            ksort($this->bs);
        } catch (\Throwable) {
            // Keep the built-in calendar available during install, migration, or DB outages.
        }
    }

    public function minBsYear(): int
    {
        return (int) min(array_column($this->bs, 0));
    }

    public function maxBsYear(): int
    {
        return (int) max(array_column($this->bs, 0));
    }

    public function yearIndex(int $year): ?int
    {
        foreach ($this->bs as $index => $row) {
            if ((int) $row[0] === $year) {
                return $index;
            }
        }
    ...
    public function getBSCal($year)
    {
        $index = $this->yearIndex((int) $year);

        return $index === null ? null : $this->bs[$index];
    }
    ...
    private function get_day_of_week($day){		
        ...
        public function NepaliBaar($day){		
        switch($day){
            case 1:
                $day = "Sunday";
                break;
    ...
    public function get_day_abbr($day){		
        switch($day){
            case 1:
                $day = "Su";
                break;
    ...
    public function NepaliBaarNP($day){		
        switch($day){
            case 1:
                $day = "आइतबार";
                break;
    ...
    private function is_range_nep($yy, $mm, $dd){		
        $yearData = $this->getBSCal((int) $yy);

        if(! $yearData){
            $this->debug_info = "Supported only between {$this->minBsYear()}-{$this->maxBsYear()}";
            return false;
        }
        ...
        $monthDays = $yearData[(int) $mm] ?? 0;

        if($dd<1 || $dd > $monthDays){
            $this->debug_info="Error! date exceeds selected BS month length";	
            return false;
        }		
    ...
    public function ad_2_bs($yy,$mm,$dd){
        ...
        if ($this->is_range_eng($yy, $mm, $dd) == false){
            ...
            $this->nep_date["month"] = sprintf('%02d', $m);
            $this->nep_date["date"] = sprintf('%02d', $total_nDays);
     ...
     * @return unknown
     */
    public function bs_2_ad($yy,$mm,$dd){
