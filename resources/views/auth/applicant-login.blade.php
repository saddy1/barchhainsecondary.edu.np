...
<head>
    ...
    <title>Applicant Login | {{ $siteSettings->localized("site_name", "Barchhain Secondary School") }} Jobs</title>
...
<div class="w-full max-w-md">
    <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-bold text-[#0b2415]">Job Applicant Login</h1>
        <p class="text-gray-500 text-sm mt-1">Sign in to apply for vacancies</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <form method="POST" action="{{ route('applicant.login.submit') }}" class="space-y-5">
            @csrf
            ...
            <div class="flex items-center">
                <input id="remember" type="checkbox" name="remember" class="w-4 h-4 text-[#1a5632] rounded border-gray-300">
                <label for="remember" class="ml-2 text-sm text-gray-600">Remember me</label>
            </div>

            <button type="submit"
                class="w-full py-4 bg-[#1a5632] text-white font-bold rounded-xl hover:bg-[#0b2415] hover:shadow-lg transition-all duration-300 text-base">
                Sign In
            </button>
        </form>
        ...
        <p class="text-center text-sm text-gray-500 mt-6">
            No account yet?
            <a href="{{ route('register') }}" class="text-[#1a5632] font-bold hover:underline">Create one free</a>
        </p>
    </div>
